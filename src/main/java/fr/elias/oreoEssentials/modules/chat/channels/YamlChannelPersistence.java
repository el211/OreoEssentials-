...
/**
 * YAML-based persistence for player channel preferences
 * Stores data locally - does NOT sync across servers
 */
public class YamlChannelPersistence implements ChannelPersistenceProvider {

    private final OreoEssentials plugin;
    private final File dataFile;

    public YamlChannelPersistence(OreoEssentials plugin) {
        this.plugin = plugin;
        ...
        if (!dataFile.exists()) {
            try {
                ...
                plugin.getLogger().info("[Channels] YAML persistence initialized (local server only)");
            } catch (IOException e) {
