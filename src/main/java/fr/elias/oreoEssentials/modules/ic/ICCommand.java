...
public final class ICCommand implements TabExecutor {
    ...
    public boolean onCommand(CommandSender s, Command cmd, String label, String[] a) {
        ...
        if (a.length == 0) {
            ...
            s.sendMessage("§e/ic reload §7| reload from file");
        ...
        if (sub.equals("addblock") && a.length >= 2) {
            if (!(s instanceof Player p)) { s.sendMessage("§cPlayer-only."); return true; }
        ...
        if (sub.equals("addentity") && a.length >= 2) {
            ...
            ICEntry e = require(mgr, s, a[1]); if (e == null) return true;
            e.entities.add(eHit.getUniqueId());
            mgr.save();
        ...
        if (sub.equals("public") && a.length >= 3) {
            ICEntry e = require(mgr, s, a[1]); if (e == null) return true;
            e.isPublic = Boolean.parseBoolean(a[2]);
            mgr.save();
            s.sendMessage("§aPublic set to §f" + e.isPublic + "§7. Players with §6oreo.ic.sign §7can place signs like §f[ic:" + e.name + "]");
            return true;
        }
    ...
    public List<String> onTabComplete(CommandSender s, Command cmd, String alias, String[] a) {
        ...
        if (a.length == 1) return starts(Arrays.asList("new", "addblock", "addentity", "addcmd", "public", "list", "info", "reload"), a[0]);
        if (a.length == 2 && !a[0].equalsIgnoreCase("new") && !a[0].equalsIgnoreCase("list") && !a[0].equalsIgnoreCase("reload"))
            return starts(mgr.all().stream().map(e -> e.name).collect(Collectors.toList()), a[1]);
