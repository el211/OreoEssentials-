package fr.elias.oreoEssentials.modules.holograms.commands;
...
import fr.elias.oreoEssentials.modules.holograms.storage.converter.ConverterTarget;
...
public final class OHologramsCMD extends Command {

    @NotNull
    private final OHolograms plugin;

    public OHologramsCMD(@NotNull final OHolograms plugin) {
        super("oholograms");
        setPermission("oholograms.admin");
        this.plugin = plugin;
    }
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String label, @NotNull String[] args) {
        ...
        switch (args[0].toLowerCase(Locale.ROOT)) {
            ...
            case "version" -> OHolograms.get().getHologramThread().submit(() -> OHolograms.get().getVersionConfig().checkVersionAndDisplay(sender, false));
            case "convert" -> {
                if (args.length < 3) {
                    MessageHelper.info(sender, "Usage: /oholograms convert <type> <targets> [args...]");
                    return false;
                }
