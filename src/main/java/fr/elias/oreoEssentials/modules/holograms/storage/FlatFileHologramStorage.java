package fr.elias.oreoEssentials.modules.holograms.storage;
    ...
    private List<Hologram> readHolograms(@NotNull File configFile, @Nullable String world) {
        ...
        try {
            ...
            if (!config.isConfigurationSection("holograms")) {
                OHolograms.get().getFancyLogger().warn("No holograms section found in config");
                return new ArrayList<>(0);
            }

            int configVersion = config.getInt("version", 1);
            if (configVersion != 2) {
                OHolograms.get().getFancyLogger().warn("Config version is not 2, skipping loading holograms");
                OHolograms.get().getFancyLogger().warn("Old config version detected, skipping loading holograms");
                return new ArrayList<>(0);
            }
            ...
            for (String name : hologramsSection.getKeys(false)) {
                ...
                if (holoSection == null) {
                    OHolograms.get().getFancyLogger().warn("Could not load hologram section in config");
                    continue;
                }
                ...
                if (typeName == null) {
                    OHolograms.get().getFancyLogger().warn("HologramType was not saved");
                    continue;
                }

                HologramType type = HologramType.getByName(typeName);
                if (type == null) {
                    OHolograms.get().getFancyLogger().warn("Could not parse HologramType");
                    continue;
                }
                ...
                if (!displayData.read(holoSection, name)) {
                    OHolograms.get().getFancyLogger().warn("Could not read hologram data - skipping hologram");
                    continue;
                }
