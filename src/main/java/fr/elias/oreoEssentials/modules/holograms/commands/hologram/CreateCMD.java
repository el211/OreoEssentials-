...
import fr.elias.oreoEssentials.modules.holograms.api.data.*;
...
public class CreateCMD implements Subcommand {
    ...
    public boolean run(@NotNull CommandSender sender, @Nullable Hologram hologram, @NotNull String[] args) {
        ...
        if (!(sender.hasPermission("OHolograms.hologram.create"))) {
        ...
        final var holo = OHolograms.get().getHologramsManager().create(displayData);
        ...
        for (Player onlinePlayer : Bukkit.getOnlinePlayers()) {
            holo.forceUpdateShownStateFor(onlinePlayer);
        }
