// File: src/main/java/fr/elias/oreoEssentials/modgui/menu/WorldBannedMobsMenu.java
...
public class WorldBannedMobsMenu implements InventoryProvider {
    ...
    // Comprehensive list of all spawnable Minecraft mobs
    ...
    // Spawn egg materials for each entity type
    private static final Map<EntityType, Material> EGG = Map.ofEntries(
            ...
            Map.entry(EntityType.WARDEN, Material.REINFORCED_DEEPSLATE), // No spawn egg
            Map.entry(EntityType.BREEZE, Material.TRIAL_SPAWNER), // No spawn egg yet
    ...
    public void init(Player p, InventoryContents c) {
        ...
        for (EntityType t : CATALOG) {
            ...
            if (++col >= 8) {
                col = 1;
                row++;
                if (row >= 5) break; // Don't overflow GUI
            }
        }
    }
     ...
     * Uses modern Paper API when available, falls back to hidden enchant trick.
     */
