package fr.elias.oreoEssentials.currency.commands;

import fr.elias.oreoEssentials.OreoEssentials;
import fr.elias.oreoEssentials.currency.Currency;
...
/**
 * Tab completer for /oecurrency command
 * Supports cross-server player suggestions via PlayerDirectory (Mongo)
 */
public class CurrencyCommandTabCompleter implements TabCompleter {
    ...
    public List<String> onTabComplete(
        ...
        switch (sub) {
            ...
            case "create" -> {
                if (args.length == 2) {
                    // currency id → no suggestions on purpose
                    return List.of();
                }
                if (args.length == 3) {
                ...
                if (args.length == 4) {
                    // keep ASCII-friendly ones (your choice)
    ...
    private Collection<String> getNetworkPlayerNames(CommandSender sender, String prefixRaw) {
        ...
        // Always include local server players
        for (Player p : Bukkit.getOnlinePlayers()) {
            suggestions.add(p.getName());
        }

        // Add network players (Mongo)
