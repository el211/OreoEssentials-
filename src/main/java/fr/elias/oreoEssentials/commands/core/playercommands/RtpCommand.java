    ...
    public class RtpCommand implements OreoCommand {
        ...
        public boolean execute(CommandSender sender, String label, String[] args) {
            ...
            if (cfg == null || !cfg.isEnabled()) {
                Lang.send(p, "rtp.disabled", null, null);
                return true;
            }
            ...
            if (targetWorldName == null || targetWorldName.isBlank()) {
                Lang.send(p, "rtp.no-world", null, null);
                return true;
            }
            ...
            new org.bukkit.scheduler.BukkitRunnable() {
                ...
                public void run() {
                    ...
                    if (hasMoved(target, origin)) {
                        cancel();
                        Lang.send(target, "rtp.cancelled-moved", null, null);
                        return;
                    }
                    ...
                    String title = Lang.msg("rtp.warmup.title", "&eTeleporting in %seconds%s...", target)
                            .replace("%seconds%", String.valueOf(remaining));

                    String subtitle = Lang.msg("rtp.warmup.subtitle", "&7World: &f%world% &8• &7in &e%seconds%s", target)
                            .replace("%seconds%", String.valueOf(remaining))
        ...
        public static boolean doLocalRtp(OreoEssentials plugin,
            ...
            RtpConfig cfg = plugin.getRtpConfig();
            if (cfg == null) {
                Lang.send(p, "rtp.not-configured", null, null);
                return false;
            }
            ...
            if (!cfg.allowedWorlds().isEmpty() && !cfg.allowedWorlds().contains(world.getName())) {
                Lang.send(p, "rtp.not-allowed-world", null, null);
                return false;
            }
            ...
            if (dest == null) {
                Lang.send(p, "rtp.no-safe-spot", null, null);
                return false;
            }
            ...
            if (ok) {
                ...
                Lang.send(p, "rtp.failed", null, null);
            }
            return ok;
        }
